...
const HeaderPanel = (props) => {
  ...
  useEffect(() => {
    fetch('http://18.207.251.49:4006/status/ftm').then(res => res.json()).then(result => {
  ...
  const handleGetFTM = () => {
    ...
    fetch('http://18.207.251.49:4006/request/ftm/' + targetAddress).then(res => res.json()).then(result =>{
    ...
    <div className="headerContainer">
      ...
      <div className = "faucetContainer">
        <FormControl className = 'ftmAddressInput'  variant="filled">
            ...
            <FormHelperText className = 'helper' id="filled-weight-helper-text">{displayMsg}</FormHelperText>
            <FormHelperText className = 'helper' id="filled-weight-helper-text">Test Fantoms are served from <b>{address}</b> with <b>{ftm}</b> FTMs, 10 tFTMs each time.</FormHelperText>
          </FormControl>
      </div>
